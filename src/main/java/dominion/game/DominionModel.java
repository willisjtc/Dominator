...
//		setPlayerTurn(random.nextInt(players.size()));
	...
	public Result buyCard(Player player, Card card) {
...
//		if (players.get(playerTurn).equals(player)) {
			result = playerCanBuy(players.get(playerTurn), card);
			if (kingdomCardInGame(card)) {
				if (result.isSuccess()) {
					KingdomCard kingdomCard = getKingdomCard(card);
					player.addToDiscard(kingdomCard.drawCard());
...
//		}
	...
	private Result playerCanBuy(Player player, Card card) {
		Result result = new Result(false, "You can't afford that card");
		int playerGold = 0;
		for (Card cardInHand : player.getHand()) {
			if (cardInHand.isTreasure()) {
				try {
					playerGold += cardInHand.getValue();
				} catch (Exception e) {
					log.log(Level.WARNING, "Card does not have getValue() method", e);
				}
			}
		}
		if (playerGold >= card.getCost()) {
			result.setSuccess(true);
			result.setMessage("You can afford that card");
		}
		return result;
	}
